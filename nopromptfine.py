...
def preprocess_function(examples):
    ...
    inputs = [
        f"{clean_text(question)} {EOS_TOKEN} {clean_text(answer)}"
...
# 데이터셋 전처리
processed_datasets = dataset.map(preprocess_function, batched=True, remove_columns=["question", "answer"])

# 데이터셋 분할 (80% 훈련, 20% 검증)
train_eval_split = processed_datasets['train'].train_test_split(test_size=0.2, seed=42)
...
plt.savefig("training_eval_loss_sampled_v2.png", dpi=300, bbox_inches='tight')
plt.close()
